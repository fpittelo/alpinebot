...
data "azuread_group" "qa_admins" {
  display_name = "qa_admins"
}

# Data block to retrieve the Key Vault details
data "azurerm_key_vault" "qa_kv" {
  name                = var.az_kv_name
...
resource "azurerm_role_assignment" "qa_admins_kv_secrets_officer" {
  scope                = data.azurerm_key_vault.qa_kv.id
  role_definition_name = "Key Vault Secrets Officer"  # Predefined role in Azure
  principal_id         = data.azuread_group.qa_admins.object_id
}

# Assign the "Key Vault Administrator" role to the Dev_Admins group using RBAC
resource "azurerm_role_assignment" "qa_admins_kv_administrator" {
  scope                = data.azurerm_key_vault.qa_kv.id
